...
public class TestUsuarioService {
    ...
    void setUp() throws Exception {
        // Create a path to a temporary file inside the temporary directory.
        File tempFile = tempDir.resolve("test_usuarios.ser").toFile();
        tempFilePath = tempFile.getAbsolutePath();

        // Use reflection to change the public static final field `ARQUIVO_USUARIOS`.
        // This allows us to redirect the service's file operations to our temp file.
        Field field = UsuarioService.class.getDeclaredField("ARQUIVO_USUARIOS");
        field.setAccessible(true);
        // We can't change a final field directly, but we can set it to our temp path.
        // In this context for testing, this is an acceptable workaround.
        // For more complex scenarios, frameworks like PowerMock might be used.
        setFinalStatic(field, tempFilePath);

        // A new service instance is created for each test, ensuring isolation.
    ...
    @AfterEach
    void tearDown() {
        // Clean up the created file after each test to be safe.
